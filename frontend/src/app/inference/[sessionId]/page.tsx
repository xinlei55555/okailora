...
export default function InferencePage() {
	...
	const renderNavigationBar = () => {
		...
		const specialButtonClass =
			'bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 text-white px-8 py-3 rounded-lg transition-all font-semibold';
				...
				return (
					<div className="flex justify-between">
						...
						{uploadedFiles.some((f) => f.status === 'ready' || f.status === 'completed') && (
							<button onClick={handleStartInference} className={specialButtonClass}>
								🎯 Run Inference
							</button>
						)}
					</div>
	...
	return (
		<div className="min-h-screen bg-gray-950 text-white p-8">
			<div className="max-w-7xl mx-auto">
				...
				<div className="mt-8 pt-8 border-t border-gray-800">{renderNavigationBar()}</div>
			</div>
		</div>
	);
}
